...
// --- UNIVERSAL PROCESSING LOADER & DUPLICATE-CLICK PREVENTION ---
(function() {
  ...
  window.fetch = async function(...args) {
    ...
    try {
      ...
      if (activeRequestsCount <= 0) {
        ...
        hideMobileLoading();
        document.querySelectorAll('button:disabled[data-auto-disabled="true"], input[type="submit"]:disabled[data-auto-disabled="true"]').forEach(el => {
          el.disabled = false;
          delete el.dataset.autoDisabled;
        });
      }
    }
  };
  ...
  document.addEventListener('click', (e) => {
    ...
    if (btn && !btn.disabled) {
      if (btn.classList.contains('close-btn') || btn.getAttribute('onclick')?.includes('close') || btn.getAttribute('onclick')?.includes('toggle')) {
        return;
      }
      if (btn.classList.contains('btn-primary') || btn.classList.contains('btn-cta') || btn.classList.contains('btn-add-tile') || btn.type === 'submit' || btn.closest('form')) {
        if (!btn.dataset.autoDisabled) {
          btn.dataset.autoDisabled = 'true';
          btn.disabled = true;
          showMobileLoading();

          setTimeout(() => {
            if (btn.dataset.autoDisabled === 'true') {
              btn.disabled = false;
  ...
  document.addEventListener('submit', (e) => {
    ...
    if (submitBtn && !submitBtn.disabled) {
      ...
      submitBtn.dataset.autoDisabled = 'true';
      showMobileLoading();
    }
  }, true);
})();
